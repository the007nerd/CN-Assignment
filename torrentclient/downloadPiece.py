...
def get_torrent_info(file_path):
    ...
    with open(file_path, "rb") as file:
        ...
        # Calculate the list of all piece hashes of 20 bytes
        piece_hashes = [
...
def send_request_message(sock, index, begin, length):#send peer message to peer
    
    message_id = b'\x06'
    payload = (
        index.to_bytes(4, byteorder='big') +    #it is of 4 bytes each so convert all these into 4 bytes
